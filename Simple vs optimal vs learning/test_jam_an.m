function [n_jam_left_out, rep_g_out, rep_m_out,pr_out,seq_out,p1,p2,p3]=test_jam_an(Nmax,q,g,rep_g,rep_m,p1m,p1g,rep_vector, n_jam_left);

p1=0;
p2=0;
p3=0;
rep_g_out=zeros(4^Nmax,length(rep_g));
rep_m_out=zeros(4^Nmax,length(rep_m));
p_out=zeros(4^Nmax,1);
n_jam_left_out=zeros(4^Nmax,1);
seq_out=-ones(4^Nmax,Nmax);
...
actions_g=ones(1,sum(rep_g)); %1 is NO JAM
actions_g(1:n_jam_left)=0; % 0 is JAM
actions_a=2*ones(1,sum(rep_m)); %2 is ATTACK

while il<=4^Nmax-1
    njl=n_jam_left;
    jv=zeros(Nmax,1); % 1 if there is a node jammed!
    ilaux=-1;
    seq=dec2base(il,4,Nmax)-'0'; % 0: 1 good, 1: 0 good, 2: 1 malicious, 3: 0 malicious
    Wp=zeros(Nmax,1);
    %Initialize p_seq
    seq_index=1;
    ...
    while seq_index<=Nmax
        ...
        if sum(Wp)>=q || sum(Wp)<=-q || seq_index==Nmax % Out!
            ...
            seq_index=Nmax+1; %To exit the while
        else
