...
class DoubleDQNAgent: # Define the agent that is going to be used for training
    def __init__(self, state_size, EPISODES,N, njam_max,Ma,Mm):
        ...
        self.trace_length=4
        self.state_features=state_size
    ...
    def build_model(self):
        # We build a model with 2 layers
        model = Sequential()
    ...
    def get_action(self, state):
        statee=np.reshape(state[0,-1,:],[1,self.state_size])
        njam=int(statee[0,-1])
    ...
    def train_model(self):
        ...
        for i in range(self.batch_size):
            ...
            if done[i]:
                ...
                upd=update_target[i,-1,:]
                njam=int(upd[-1])
                actions_actual=self.actions_total[njam]
                a = np.argmax(target_val[i,0:len(actions_actual)])
    ...
    def empirical_simulate(self,Mm,q,g,initial_state,Pc,u,N,Ma):
        rwd = 0
        state =np.zeros([1,self.trace_length,self.state_size])
        ...
        for stage in range(N):
            ...
            next_statee, reward = self.transition(Mm,q,g,statee,action_v,Pc,u,N)
            rwd=rwd+reward/N
            
            next_state = np.zeros([1,self.trace_length,self.state_size])
            next_state[0,0:self.trace_length-1,:]=state[0,1:,:]
            next_state[0,-1,:]=next_statee
            state = next_state
        return rwd
...
if __name__ == "__main__":
    def processinput(Pc,u,nj):
        
        nsm=dic[str(nj)+" "+str(Pc)+" "+str(u)]
        if os.path.isfile("./python_data/DRQN_training"+str(nsm)+".h5")==True: #NN trained already
            ...
            for e in range(EPISODES):
                ...
                score = 0
                state =np.zeros([1,agent.trace_length,state_size])
                state[0,-1,:]=initial_state
                ...
                while not done: # Iterate while the game has not finished
                    ...
                    statee=np.reshape(state[0,-1,:],[1,state_size])
                    action = agent.get_action(state) # Using epsilon-greedy policy
                    nj_id=int(statee[0,-1]) # Use only possible actions!!
                    actions_actual=agent.actions_total[nj_id]
                    action_v=actions_actual[action]
                    next_statee, reward = agent.transition(Mm,q,g,statee,action_v,Pc,u,N)
                    reward=reward/N # Reward averaged through stages
                    if stage==N:
                        done=True #Final state!!
                    next_state = np.zeros([1,agent.trace_length,state_size])
                    next_state[0,0:agent.trace_length-1,:]=state[0,1:,:]
                    next_state[0,-1,:]=next_statee
            ...
            agent.save("./python_data/DRQN_training"+str(nsm)+".h5")
            ...
            sio.savemat("./python_data/DRQN_"+str(nsm)+".mat",out_values)
    ...
    #results = Parallel(n_jobs=num_cores, verbose=5)(delayed(processinput)(Pc=k,u=l,nj=m) for k in Pcv for l in uv for m in njv)
    processinput(0.0,0,0)